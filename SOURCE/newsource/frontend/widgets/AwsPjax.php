...
class AwsPjax extends Widget
{
    ...
    public function registerClientScript()
    {
        ...
        if ($this->linkSelector !== false) {
            ...
            $js .= "$(document).pjax($linkSelector, \"#$id\", $options);";
        }
        if ($this->formSelector !== false) {
            $formSelector = Json::htmlEncode($this->formSelector !== null ? $this->formSelector : '#' . $id . ' form[data-pjax]');
            $js .= "\n$(document).on('submit', $formSelector, function (event) {\$.pjax.submit(event, '#$id', $options);});";
        }
        $view = $this->getView();
        AwsPjaxAsset::register($view);
